...
const isAnchorLink = (link: any) => {
  ...
  const convertLinkToFullPath = (link: any) => {
  ...
  */
  function siteGraphGenerator(sites: any, pages: any) {
    const errors: any = []
    const siteMap = sites.reduce((map: any, link: any) => {
      map[cleanSiteLink(link)] = true
      return map
    }, {})
    const siteNodes = pages.map((node: any) => {
      return {
    ...
    siteNodes.forEach((node: any) => {
      ...
      if (node.links) {
        ...
        node.links.forEach((link: any) => {
          if (isAnchorLink(link)) return; // skip anchor links, use bash script validator instead
    ...
    const recursiveTreeSetToTreeArray: any = (tree: any) => {
